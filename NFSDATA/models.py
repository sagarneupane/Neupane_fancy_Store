...
AGE_GROUPS = (
    ("INFANTS", "Below 5"), ("MID_INFANTS", "6-10"), ("TEENAGER", "11-18"), ("WORKING", "19-55"),
    ("DEPENDENTS", "Over 56"))
...
class Products(models.Model):
    ...
    product_category = models.ForeignKey(Category, on_delete=models.SET_NULL, null=True, default=1)
    product_subcategory = models.ForeignKey(SubCategory, on_delete=models.SET_NULL, null=True, default=1)
...
class Bills(models.Model):
    bill_no = models.IntegerField(primary_key=True, auto_created=True)
    items = models.ManyToManyField(SoldProducts)
    billing_date = models.DateField(auto_now=True)
    customer_detail = models.CharField(max_length=255)
    customer_contact = models.CharField(max_length=15)

    def items_in_bill(self):
        return [str(item) for item in self.items.all()]

    def total_item_price(self):
        total_cost = 0
        for item in self.items.all():
            total_cost = total_cost + item.product_selling_price * item.product_amount_sold
        return total_cost
